...
from agents.company_agent.agent_core.embedding import embed_query, embed_texts, load_embedding_config
...
def build_docs_index(*, agent_root: Path | None = None) -> dict[str, Any]:
    ...
    if entries:
        try:
            from agents.company_agent.agent_core.embedding import clear_model_cache
...
def query_docs(question: str, *, agent_root: Path | None = None) -> dict[str, Any]:
    """Grounded docs answer from RAG chunks."""
    result = retrieve_docs(question, agent_root=agent_root)
    if not result.get("ok"):
        return {"ok": False, "answer": "No docs index found — run build_context first.", **result}
    chunks = result.get("chunks") or []
    if not chunks:
        return {"ok": True, "answer": "No relevant documentation found.", "sources": []}
